...
|128
|R_LARCH_PCADD_HI20
|[31 ... 12] bits of 32 PC-relative offset, suitable for `pcaddu12i` instead of `pcalau12i`
|`+(*(uint32_t *) PC) [24 ... 5] = (S+A-PC+0x800) [31 ... 12],+`

|129
|R_LARCH_PCADD_GOT_HI20
|[31 ... 12] bits of 32-bit PC-relative offset to GOT entry, suitable for `pcaddu12i` instead of `pcalau12i`
|`+(*(uint32_t *) PC) [24 ... 5] = (GOT+G-PC+0x800) [31 ... 12],+`

|130
|R_LARCH_PCADD_TLS_IE_HI20
|[31 ... 12] bits of 32-bit PC-relative offset to TLS IE GOT entry, suitable for `pcaddu12i` instead of `pcalau12i`
|`+(*(uint32_t *) PC) [24 ... 5] = (GOT+IE-PC+0x800) [31 ... 12],+`

|131
|R_LARCH_PCADD_TLS_DESC_HI20
|[31 ... 12] bits of 32-bit PC-relative offset to TLS descriptor, suitable for `pcaddu12i` instead of `pcalau12i`
|`+(*(uint32_t *) PC) [24 ... 5] = (GOT+DESC-PC+0x800) [31 ... 12],+`

|132
|R_LARCH_PCADD_LO12_I
|[11 ... 0] bits of 32-bit PC-relative offset to the target of an `R_LARCH_PCADD_*_HI20` reloc filling the immediate slot for a `pcaddu12i` instruction at `S`; the addend of `R_LARCH_PCADD_LO12_I` must be 0
...
|R(S)
|The target (it may be a symbol, a GOT entry for a non-TLS or TLS IE symbol, or a TLS descriptor for a TLS LD/GD symbol) of an `R_LARCH_PCADD_*_HI20` reloc filling the immediate slot for a `pcaddu12i` instruction at S
|===
